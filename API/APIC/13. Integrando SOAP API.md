...
Modificar la aplicacion para soportar HTTPS (proxy reverso)
=

Instalar modulos en Apache
...
Creamos /etc/httpd/conf.d/proxy_apic.conf

```
# Proxy para la API SOAP/APIC (HTTPS)
ProxyRequests Off
ProxyPreserveHost On

# ProxyPass para la ruta /api_apic a la URL HTTPS real
ProxyPass "/api_apic/" "https://api.us-east-a.apiconnect.ibmappdomain.cloud/api-connect-dn-1/sandbox/historialclientesservice/"
ProxyPassReverse "/api_apic/" "https://api.us-east-a.apiconnect.ibmappdomain.cloud/api-connect-dn-1/sandbox/historialclientesservice/"

# Opcional: permitir CORS para esta ruta
<Directory "/var/www/html">
    Header set Access-Control-Allow-Origin "*"
    Header set Access-Control-Allow-Methods "GET,POST,OPTIONS"
    Header set Access-Control-Allow-Headers "Content-Type, X-IBM-Client-Id"
</Directory>
...
<head>
<meta charset="UTF-8" />
<title>Consulta Historial Clientes vía Proxy</title>
<style>
  body { font-family: Arial, sans-serif; margin: 20px; }
  #resultado { margin-top: 15px; white-space: pre-wrap; background: #f0f0f0; padding: 10px; border-radius: 5px; }
</style>
</head>
<body>
  <h1>Historial Clientes (sin ID)</h1>
  <button id="btnConsultar">Consultar Historial</button>
  ...
  <script>
    document.getElementById('btnConsultar').addEventListener('click', async () => {
      ...
      try {
        const response = await fetch('/api_apic/getHistorialPorId', {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          out.textContent = `Error en la respuesta: ${response.status} ${response.statusText}`;
          return;
        }
        ...
        if (data && data.return && data.return.item && data.return.item.length > 0) {
          let texto = 'Historial completo:\n\n';
          data.return.item.forEach(entry => {
            const fecha = entry.fecha ? entry.fecha['$'] || entry.fecha : 'sin fecha';
            const accion = entry.accion ? entry.accion['$'] || entry.accion : 'sin acción';
            texto += `• ${fecha} - ${accion}\n`;
          });
          out.textContent = texto;
        } else {
          out.textContent = 'No se encontraron registros en el historial.';
        }
      } catch (error) {
        out.textContent = 'Error al llamar al proxy: ' + error.message;
      }
    });
  </script>
</body>
