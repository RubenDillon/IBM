Integrando una API SOAP (opcional) **SOAP-to-REST proxy**
=
...
NOTA: 
-
Para continuar con estos pasos, hay que poner HTTPS al servidor front end... por favor seguir los pasos de la instalacion de Vault y como habilitar PKI, para que el servidor frontend pueda publicar a traves de HTTPS. Esto es necesario, ya que APIC solo acepta HTTPS.
    ...
    document.getElementById('buscarBtn').addEventListener('click', async () => {
      ...
      try {
        const response = await fetch('/api_apic/getHistorialPorId.php', {
          method: 'GET', // si tu PHP acepta POST, si no, pon GET
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
          },
          body: null // o '{}' si tu PHP requiere body aunque vacío
        });
