...
NOTA: 
-
Para continuar con estos pasos, hay que poner HTTPS al servidor front end... por favor seguir los pasos de la instalacion de Vault (archivo **10. Usando Vault.md** de este github) y como habilitar PKI (**11. Usando PKI.Vault.md**), para que el servidor frontend pueda publicar a traves de HTTPS. Esto es necesario, ya que APIC solo acepta HTTPS.


Porque no probamos y testeamos desde la UI de APIC la API? Porque no se va a permitir la conexion entre APIC y el servidor que tiene la API SOAP, ya que esta utilizando HTTP y APIC va a conectarse via HTTPS. Basicamente...

  - Estamos accediendo a APIC en HTTPS (https://api.us-east-a.apiconnect.ibmappdomain.cloud)

  - Desde ahí, intentamos llamar a nuestro backend SOAP que está en HTTP

  - El navegador bloquea por política de seguridad esa llamada: HTTPS → HTTP no está permitido (mixed content)

Como podriamos resolverlo? Poniendo HTTPS en el servidor de API, utilizando certificados firmados publicos (no usando Vault, que entregamos certificados autofirmados)

Vamos a continuar con la configuracion actual, dejando esta opcion para un proximo paso.
