...
Para simplificar, no usaremos la API con parametros.. por lo que vamos a hacer cambios en lo que hemos desarrollado.
...
<definitions name="HistorialClientesService"
  ...
  <message name="getHistorialCompletoRequest">
    <!-- Sin parámetros -->
  </message>

  <message name="getHistorialCompletoResponse">
    <part name="historial" type="tns:HistorialArray"/>
  </message>

  <portType name="HistorialClientesPortType">
    <operation name="getHistorialCompleto">
      <input message="tns:getHistorialCompletoRequest"/>
      <output message="tns:getHistorialCompletoResponse"/>
    </operation>
  </portType>
  ...
  <binding name="HistorialClientesBinding" type="tns:HistorialClientesPortType">
    ...
    <operation name="getHistorialCompleto">
      <soap:operation soapAction="getHistorialCompleto"/>
      <input>
  ...
  <service name="HistorialClientesService">
    <documentation>Servicio para obtener historial completo de clientes</documentation>
...
class HistorialClientesService {
    public function getHistorialCompleto() {
        ...
        $result = $mysqli->query("SELECT IdCliente, fecha, accion FROM historial_clientes");
...
<?php
try {
    $client = new SoapClient("http://169.59.188.74:8080/api/soap_historial.wsdl", [
        'trace' => 1,
        'exceptions' => 1
    ]);

    $result = $client->getHistorialCompleto();
