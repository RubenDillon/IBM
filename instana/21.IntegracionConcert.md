...
Creamos un script con el siguiente curl o lo ejecutamos directamente para subir el syft
...
Para Trivy, deberiamos generar la conversion al formato que Concert espera recibir. Para ello, utilizamos este codigo python. Creamos un archivo llamado conversion.py 

NOTA: Este archivo de conversion utiliza el vm_scan_concert_sample.csv como modelo ejemplo que Concert espera cuando se envia un scan de Vulnerabilidades utilizando una herramienta que no esta listada. En el momento que escribo este tutorial, ese archivo se encuentra en https://github.com/IBM/Concert/blob/main/use-case-data-files/1.0.3/vulnerabilities/vm_scan/vm_scan_concert_sample.csv y es necesario tenerlo en la VM que usamos para revisar sus vulnerabilidades
...
csv_template_file = "vm_scan_concert_sample.csv"
...
# 1. Leer columnas del CSV de ejemplo
with open(csv_template_file, newline='', encoding='utf-8') as f:
    reader = csv.reader(f)
    headers = next(reader)  # Cabeceras reales del CSV
...
# 3. Recorrer vulnerabilidades y mapear usando nombres EXACTOS del CSV
for result in data.get("Results", []):
    for vuln in result.get("Vulnerabilities", []):
        row = {col: "" for col in headers}  # inicializar todas las columnas vacías

        # Mapeo explícito según definición
        if "CVE" in row: row["CVE"] = vuln.get("VulnerabilityID", "")
        if "Host IPAddress" in row: row["Host IPAddress"] = host_ip
        if "Package" in row: row["Package"] = vuln.get("PkgName", "")
        if "Package Version" in row: row["Package Version"] = vuln.get("InstalledVersion", "")
        if "Package Path" in row: row["Package Path"] = vuln.get("PkgIdentifier", {}).get("PURL", "")
        if "severity" in row: row["severity"] = vuln.get("Severity", "")
        if "Severity" in row: row["Severity"] = vuln.get("Severity", "")
        if "Score" in row:
            score = ""
            if "CVSS" in vuln:
            ...
            row["Score"] = score
        if "hasFix" in row: row["hasFix"] = vuln.get("FixedVersion", "")
        if "Description" in row: row["Description"] = vuln.get("Description", "")
        if "Host Name" in row: row["Host Name"] = host_name
...
print(f"✅ Conversión completada: {output_csv_file}")
...
curl -X POST "https://71538.jp-tok-2.concert.saas.ibm.com/ingestion/api/v1/upload_files" \
  ...
  -H "Content-Type: multipart/form-data" \
  -H "InstanceID: 20250802-2204-1075-61d7-381f41e7b94c" \
  -H "Authorization: C_API_KEY cnViZW4uZGlsbG9afasfwfwefwfdsaDk5xxxyxyxyxNiMDAw" \
  -F "data_type=vm_scan" \
...
y deberiamos ver una respuesta similar al previo curl (202 Acceptance)
